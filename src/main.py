...
app = FastAPI(
    ...
    description="Asistente de conocimiento interno con Retrieval Híbrido, Re-ranking opcional y Citas de Fuentes",
    version="2.1.0",
    lifespan=lifespan
)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rerank", action="store_true", help="Habilitar re-ranking con cross-encoder en modo CLI")
    ...
    if args.cli:
        ...
        if args.rerank:
            print("(Re-ranking con cross-encoder: ACTIVADO)")
        while True:
            q = input("\nPregunta: ")
            if q.lower() in ["salir", "exit", "quit"]:
                break
            res = service.answer_question(q, enable_rerank=args.rerank)
            print(f"\nRespuesta:\n{res.system_answer}\n")
